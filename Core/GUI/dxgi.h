...
namespace dxgi
{
	...
	inline FontTTF cascadiaCodeFont = FontTTF(IDR_TTF1);
	...
	HOOK(LRESULT WINAPI, WndProc, const HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if (ImGui::GetCurrentContext() != nullptr)
		{
			...
			if (RealConfig::Get()->gui.menu.enabled)
			{
				...
				WinUser::oSetCursor(::LoadCursor(NULL, win32_cursor));
				if (RealConfig::Get()->gui.menu.cover.enabled)
				{
					// 鼠标、键盘、手柄事件，全部拦截
	...
	DISPOSABLE_CALLBACK(DeviceObjectsInitialization, XOR("Present"))
	{
		...
		Application::Get()->internal.window = sd.OutputWindow;
		...
		ImGui::GetIO().Fonts->AddFontFromMemoryTTF(cascadiaCodeFont.originalData, cascadiaCodeFont.size, 16.f, NULL, ImGui::GetIO().Fonts->GetGlyphRangesChineseFull());
		...
		Application::Get()->renderer.device = device;
	}
	...
	HOOK(HRESULT, Present, IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
	{
		...
		CallbackManager::Get()->Invoke(XOR("Present"));
		ImGui_ImplDX11_NewFrame();
		ImGui_ImplWin32_NewFrame();
		ImGui::NewFrame();
		CallbackManager::Get()->Invoke(XOR("ImGui Frame"));
		ImGui::Render();
